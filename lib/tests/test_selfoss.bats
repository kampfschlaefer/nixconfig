...
@test "add source" {
    run curl --anyauth --user user:password --insecure -f -X POST --data 'title=Outside+Web&tags=&filter=&spout=spouts_rss_feed&url=http%3A%2F%2Foutsideweb%2Ffeed.atom' https://selfoss.arnoldarts.de/source
    [ $status -eq 0 ]
}
@test "check added source" {
    run curl --anyauth --user user:password --insecure -s -f https://selfoss.arnoldarts.de/sources/list
    [ $status -eq 0 ]
...
@test "can download favicons" {
    ...
    run curl --anyauth --user user:password --insecure -s -f https://selfoss/favicons/de284a31d18355923a459f030e2aa9cb.png
    [ $status -eq 0 ]
}
