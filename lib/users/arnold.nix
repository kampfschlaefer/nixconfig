{ config, pkgs, ... }:

{

  users.users.arnold = {
    isNormalUser = true;
    uid = 1000;
    group = mkOverride 100 "users";
    extraGroups = [ "wheel" ];
    initialHashedPassword = mkOverride 100 "$6$iGIV7lSJNXd63m$p9ajhxdkbGI8ttLw7Zb3ej/yDjzPuHGAvy5dxUCk8pg4Zh8wGD.Lt4K5gQ/CEdwtvN.ai.z974T9BhVZTapq6.";
    openssh.authorizedKeys.keys = [
      "ssh-rsa  arnold@xingu.arnoldarts.de"
    ];
  };

}